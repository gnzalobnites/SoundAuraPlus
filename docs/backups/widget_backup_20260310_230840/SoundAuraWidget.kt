...
class SoundAuraWidget : AppWidgetProvider() {
        ...
        fun sendAction(context: Context, action: String, playlistId: Long = -1, playlistName: String = "") {
            when (action) {
                ...
                ACTION_TOGGLE_PLAYLIST -> {
                    // Enviar broadcast para que el servicio maneje la acción
            ...
            // Actualizar todos los widgets después de la acción
            val appWidgetManager = AppWidgetManager.getInstance(context)
            val componentName = ComponentName(context, SoundAuraWidget::class.java)
            val appWidgetIds = appWidgetManager.getAppWidgetIds(componentName)
    ...
    private fun updateAppWidget(
        ...
        val intent = Intent(context, SoundAuraWidgetService::class.java).apply {
        ...
        SoundAuraWidgetViews.updateWidgetUI(context, views)
