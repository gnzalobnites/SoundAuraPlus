...
abstract class SoundAuraDatabase : RoomDatabase() {
    ...
    companion object {
        ...
        private val migration4to5 = Migration(4,5) { db ->
            ...
            // Dado que el campo name no era único anteriormente,
            // necesitamos añadir un número a los nombres no únicos
        ...
        private val migration5to6 = Migration(5, 6) { db ->
            db.execSQL("PRAGMA foreign_keys=off")
            db.execSQL("BEGIN TRANSACTION")
            db.execSQL("CREATE TABLE temp_playlist (" +
                "`id` INTEGER NOT NULL PRIMARY KEY," +
                "`name` TEXT NOT NULL," +
                "`shuffle` INTEGER NOT NULL DEFAULT 0," +
                "`isActive` INTEGER NOT NULL DEFAULT 0," +
                "`volume` REAL NOT NULL DEFAULT 1.0)")
            db.execSQL("INSERT INTO temp_playlist (name, shuffle, isActive, volume)" +
                       "SELECT name, shuffle, isActive, volume FROM playlist")
            db.execSQL("DROP TABLE playlist")
            db.execSQL("ALTER TABLE temp_playlist RENAME TO playlist")

            db.execSQL("CREATE TABLE temp_playlistTrack (" +
               ...
               "FOREIGN KEY (`trackUri`) REFERENCES `track`(`uri`) ON UPDATE CASCADE ON DELETE CASCADE)")
            db.execSQL("INSERT INTO temp_playlistTrack (playlistId, playlistOrder, trackUri)" +
                       "SELECT (SELECT id FROM playlist WHERE name = playlistName), " +
                               "playlistOrder, trackUri " +
                       "FROM playlistTrack")
            db.execSQL("DROP TABLE playlistTrack")
            db.execSQL("ALTER TABLE temp_playlistTrack RENAME TO playlistTrack")

            // Room no parece permitir establecer restricciones UNIQUE en columnas (aparte
            // de la unicidad implícita del campo primary key), por lo que el campo se
            // hace único en la declaración de la entidad a través de un índice único en 'name'.
            // Este índice se recrea aquí usando el nombre predeterminado de Room para índices
            // en lugar de usar una restricción UNIQUE 'name' para reflejar mejor el esquema
            // de base de datos autogenerado.
        ...
        // Guarda el nombre de archivo original de cada pista para poder
        // localizarla en MediaStore como alternativa si su permiso SAF
        // persistente se pierde más adelante.
        private val migration7to8 = Migration(7, 8) { db ->
            db.execSQL("ALTER TABLE track ADD COLUMN `displayName` TEXT DEFAULT NULL")
        }
    }
}
